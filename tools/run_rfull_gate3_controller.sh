...
# The dataset cache must be readable by every rank with identical contents.
# Two topologies satisfy that invariant:
#   shared      - one filesystem mounted on all nodes (default)
#   replicated  - an identical private copy on each node's local disk
# Replicated is required when the cache holds .npy files that MCore opens with
# numpy mmap: a page fault on a FUSE mount (blobfuse) is delivered as SIGBUS/
# SIGSEGV to the faulting thread, which no Python handler can catch, killing
# the rank and stranding its peers in the next collective.
RFULL_CACHE_MODE=${RFULL_CACHE_MODE:-shared}
mkdir -p "$DATA_CACHE_PATH"

if [[ "$RFULL_CACHE_MODE" == "shared" ]]; then
  # Prove the mount is shared: node 0 writes a probe, every node reads it back.
  PROBE="$DATA_CACHE_PATH/.shared-probe-$$"
  date -u +%Y-%m-%dT%H:%M:%SZ > "$PROBE"
  CACHE_SHA=$(sha256sum "$PROBE" | awk '{print $1}')
  ...
  if [[ -z "$CACHE_SHA" ]]; then
    echo "replicated cache fingerprint is empty at $DATA_CACHE_PATH" >&2
    exit 12
  fi
else
  echo "RFULL_CACHE_MODE must be 'shared' or 'replicated', got '$RFULL_CACHE_MODE'" >&2
  exit 2
fi
...
printf '{"marker":"CACHE_MODE","mode":"%s","path":"%s","fingerprint":"%s"}\n' \
  "$RFULL_CACHE_MODE" "$DATA_CACHE_PATH" "$CACHE_SHA"

for index in "${!HOST_ARRAY[@]}"; do
  host=${HOST_ARRAY[$index]}
  if (( index == 0 )); then
    observed=$(eval "$CACHE_PROOF")
  else
    observed=$(ssh -o BatchMode=yes -o StrictHostKeyChecking=no -o ConnectTimeout=20 "$host" \
      "$CACHE_PROOF" || true)
  fi
  if [[ "$observed" != "$CACHE_SHA" ]]; then
    printf '{"marker":"SHARED_CACHE_FAIL","host":"%s","node_rank":%s,"mode":"%s","expected":"%s","observed":"%s"}\n' \
      "$host" "$index" "$RFULL_CACHE_MODE" "$CACHE_SHA" "$observed"
    [[ -n "$PROBE" ]] && rm -f "$PROBE"
    exit 12
  fi
  printf '{"marker":"SHARED_CACHE_OK","host":"%s","node_rank":%s,"mode":"%s","sha256":"%s"}\n' \
    "$host" "$index" "$RFULL_CACHE_MODE" "$CACHE_SHA"
done
[[ -n "$PROBE" ]] && rm -f "$PROBE"
